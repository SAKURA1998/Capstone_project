...
while (frame_num < total_frame)
    ...
    if(frame_num > 3)
        for mic_num = 1:total_mic
            if(max(abs(h_diff_of_frames(frame_num,:,mic_num)))>0.01)
                %relative_distance
...
%                 end
                ...
                relative_distance(frame_num, mic_num) = relative_distance(frame_num - 1, mic_num);
            end
        end
        addpoints(h, frame_num, relative_distance(frame_num, 1));
        drawnow limitrate
    end
    ...
    if(frame_num - last_slide_frame>detect_offset)
        ...
        for i = 1:total_mic
            [max_val,I_max] =  max(relative_distance(frame_num - detect_offset:frame_num, i));
            [min_val,I_min] =  min(relative_distance(frame_num - detect_offset:frame_num, i));
            if(I_max > I_min)
