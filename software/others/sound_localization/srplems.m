...
efmax=max(mdist);
%%%%%modified here!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
%efs=2*(fix(efmax*fs/speedofsound));
efs=2*(fix(efmax*fs/speedofsound))+20; %%% End-fire is symmetric about the 0th sample so multiplying by 2.
%efs=801;
...
%15000 and 5000 gives a good accuracy
%1000 and 100 acceptable
randpts=1000;  %%% J0 in SRC. Depending on the size of the search volume, choose an appropriate value here (Here, 3000 is for a V_{search}= 4m x 1m x 6m) 
...
%% Functional Evaluation sub-routine (calculate SRP-PHAT value for a point in the search space):
    ...
    function [yval1,position1] = fe(bstart,bend,magiconst,mic_loc,yintpt,row1,efsintp)
      ...
      %  v=ddm(gidM);
        ...
        %row=row1+ddiffsi32;   modified here,!!!!!!!!!!!!!!!!!!!!!!!
        row=row1+ddiffsi32;
%         row(row<1) = 1;
